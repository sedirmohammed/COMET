...
class ExperimentRunner:
    ...
    def run(self):
        ...
        for feature in tqdm(self.all_polluted_features_results, desc="Processing features"):
            ...
            if self.ds_name == 'Airbnb':
                result = Parallel(n_jobs=10, prefer="threads")(
                    delayed(self.parallel_model_performance_calculation)(polluted_feature_result.get_random_seed(),
                                                                         polluted_feature_result.get_pollution_level(),
                                                                         self.ml_algorithm,
                                                                         polluted_feature_result.get_train_df(),
                                                                         polluted_feature_result.get_test_df())
    ...
    def parallel_model_performance_calculation(self, random_seed: int, pollution_level: float, experiment, train_df_polluted: DataFrame, test_df_polluted: DataFrame):
        ...
        exp = experiment(train_df_polluted, test_df_polluted, self.metadata[self.ds_name], self.error_type.get_classname(), self.pre_pollution_setting_id)
