...
def plot_performance_per_budget(pre_pollution_setting_id, df, df_optimal, df_fi_static, df_c_random, df_cleaned, df_comet_light):
    try:
    ...
    if len(df_c_random) > 0:
        ...
        df_c_random = df_c_random.drop(['experiment', 'dataset', 'original_budget', 'original_f1_score'], axis=1)

        df_c_random['used_budget'] = df_c_random.groupby(['run'])['used_budget'].cumsum()
        df_c_random.loc[df_c_random['iteration'] == 0, 'feature'] = ''
        df_c_random['std_diff'] = 0.0
        df_c_random['mean_diff'] = 0.0
        df_c_random = df_c_random.groupby('used_budget').agg(
            feature=pd.NamedAgg(column='feature', aggfunc=lambda x: ', '.join(map(str, x))),
        ...
        df_c_random = df_c_random.sort_values(by=['used_budget'], ascending=True)
        fig.add_trace(go.Scatter(x=df_c_random['used_budget'], y=df_c_random['real_f1'],
                                 mode='lines+markers',
                                 marker_color='#DE8F05',
...
def create_matplotlib_plot(df, df_c_random, df_fi_static, df_optimal, df_cleaned, df_comet_light):
    ...
    ax.plot(df_comet_light['used_budget'], df_comet_light['f1_score'], 'o-', color='#b20101', label='COMET Light', linestyle='-', marker='^')
    segments = []
    start = 0
    ...
    for start, end in segments:
        ...
        if not first_segment_plotted:
            ...
            ax.plot(segment['used_budget'], segment['predicted_f1_score'], 'o-', color='#949494', linestyle='--',
                    marker='D')
    if len(df_c_random) > 0:
        ax.errorbar(df_c_random['used_budget'], df_c_random['real_f1'], yerr=df_c_random['std'], fmt='o-',
    ...
    handles = [handles[i] for i in [0, 3, 2, 5, 1, 4, 6]]
    labels = [labels[i] for i in [0, 3, 2, 5, 1, 4, 6]]
...
DS_NAME = ['cmc.data', 'TelcoCustomerChurn.csv', 'EEG.arff', 'SouthGermanCredit.csv']
...
st.title('COMET evaluation considering the multi-error scenario')

st.sidebar.title('Select configuration')
experiment_string = st.sidebar.selectbox(
    'Select ML algorithm',
...
def load_and_filter_data(base_name, ds_name, experiment_str, database_url):
...
dynamic_greedy_cleaning_schedule = load_and_filter_data(
    'cleaning_results', ds_name, experiment_string, DATABASE_URL)

comet_light_cleaning_schedule = load_and_filter_data(
    'comet_light_cleaning_results', ds_name, experiment_string, DATABASE_URL)

dynamic_greedy_cleaning_schedule_optimal = load_and_filter_data(
    'cleaning_schedule_optimal', ds_name, experiment_string, DATABASE_URL)

static_feature_importance_greedy_cleaning_schedule = load_and_filter_data(
    'cleaning_schedule_static_features_importance', ds_name, experiment_string, DATABASE_URL)
completely_random_cleaning_schedule = load_and_filter_data(
    'cleaning_schedule_completely_random', ds_name, experiment_string, DATABASE_URL)

cleaned_data = load_and_filter_data(
    'cleaned_data_results', ds_name, experiment_string, DATABASE_URL)
...
for pre_pollution_setting_id in pre_pollution_setting_ids[0:3]:
    ...
    if not static_feature_importance_greedy_cleaning_schedule.empty:
        ...
        if 'real_f1' in static_feature_importance_greedy_cleaning_schedule_filtered.columns:
            static_feature_importance_greedy_cleaning_schedule_filtered.rename(columns={'real_f1': 'f1_score'}, inplace=True)
    else:
    ...
    plot_performance_per_budget(pre_pollution_setting_id,
