...
def write_feature_prediction_results_to_db(cleaning_configs, experiment_results, db_engine, iteration, ds_name, ml_algorithm_str, error_type_str, pre_pollution_setting_id):
    for feature in cleaning_configs:
        merged_df = pd.merge(experiment_results[feature], cleaning_configs[feature]['data'], on='pollution_level')
        merged_df['predicted_f1'] = merged_df['predicted_f1_y']
        merged_df['lower_confidence_border'] = merged_df['lower_confidence_border']
        merged_df['upper_confidence_border'] = merged_df['upper_confidence_border']
        merged_df['real_f1'] = merged_df['real_f1_x']
        ...
        merged_df = merged_df.drop(columns=columns_to_drop)
        merged_df = merged_df.drop(columns=['random_seed'])

        min_pollution_row = cleaning_configs[feature]['data'].loc[
            cleaning_configs[feature]['data']['pollution_level'].idxmin()]
        min_pollution_row = min_pollution_row[merged_df.columns]
        merged_df = pd.concat([merged_df, min_pollution_row], ignore_index=True)

        metadata = MetaData(bind=db_engine)
        table_name = f'cleaning_results_per_feature_{ds_name}_{ml_algorithm_str}_{error_type_str}_{feature}_{pre_pollution_setting_id}'
        cleaning_results_df = merged_df
        cleaning_results_df['iteration'] = iteration
        for attempt in range(1):
            try:
                ...
                sql_df = pd.DataFrame(results, columns=table.columns.keys())
                if iteration == 1:
                    cleaning_results_df.to_sql(table_name, con=db_engine, if_exists='replace', index=False)
                    break
                else:
                    concatenated_df = pd.concat([cleaning_results_df, sql_df], ignore_index=True)
                    concatenated_df.to_sql(table_name, con=db_engine, if_exists='replace', index=False)
                    break
...
def main(ml_algorithm, error_type, ds_name: str, original_budget, metadata, database_engine, pre_pollution_setting_ids) -> None:
    ...
    table_name = f'cleaning_results_{ds_name}_{ml_algorithm.__name__}'
    ...
    for pre_pollution_setting in pre_pollution_settings:
        ...
        if not os.path.exists(f'{ROOT_DIR}/slurm/dynamic_greedy/RESULTS/'):
            os.makedirs(f'{ROOT_DIR}/slurm/dynamic_greedy/RESULTS/')
        else:
            if os.path.exists(f'{ROOT_DIR}/slurm/dynamic_greedy/RESULTS/{table_name}_{pre_pollution_setting_id}.csv'):
                os.remove(f'{ROOT_DIR}/slurm/dynamic_greedy/RESULTS/{table_name}_{pre_pollution_setting_id}.csv')

    time_tracker = {}
    for pollution_setting in pre_pollution_settings:
        print(f'Start dynamic greedy for {ds_name}, {ml_algorithm.__name__}, multiple errors, pre-pollution setting {pollution_setting["pre_pollution_setting_id"]}')
        ...
        #pollution_setting = {'pre_pollution_setting_id': pre_pollution_setting_id, 'train': {'Number of children': 0.01, 'Wife\'s age': 0.01}, 'test': {'Number of children': 0.01, 'Wife\'s age': 0.01}}
        ...
        while BUDGET >= 0:
            ...
            if cleaning_result.get_feature() is not None:
                ...
                # write_feature_prediction_results_to_db(cleaning_result.get_error_type_name(), experiment_results, database_engine, iteration,
                #                                        ds_name, ml_algorithm.__name__, cleaning_result.get_error_type_name(),
