...
class SGDClassifierExperimentSVM(ClassificationExperiment):
    ...
    def calculate_importance_score(self, encoder, row, ds_name, delta_x, delta_y, dirty_indexes_per_feature, dirty_record_index):
        ...
        if y * decision_function <= 1:
            gradient = -y * x.flatten()  # Ensure gradient is a flat array if you're returning it
        else:
...
class SGDClassifierExperimentLinearRegression(ClassificationExperiment):
    ...
    def calculate_importance_score(self, encoder, row, ds_name, delta_x, delta_y, dirty_indexes_per_feature, dirty_record_index):
        ...
        # Calculate Mx according to your formula
        for i in range(n_features):
        ...
        # Ensure gradient is in the correct shape, if necessary
        gradient = gradient.reshape(-1, 1)
...
def write_result_to_db(iteration, ds_name, ml_algorithm_str, error_type_name, pre_pollution_setting_id, db_engine, number_of_cleaned_cells, used_budget, f1_score):
    ...
    table_name = f'activeclean_results_{ds_name}_{ml_algorithm_str}'
...
def main(ml_algorithm_class, error_type, ds_name: str, original_budget, metadata, database_engine, pre_pollution_setting_ids) -> None:
    ...
    table_name = f'activeclean_results_{ds_name}_{ml_algorithm_class.__name__}'
    ...
    for pollution_setting in pre_pollution_settings:
        ...
        ml_algorithm = ml_algorithm_class(metadata, 'multi_error', pre_pollution_setting_id)
        ...
        absolute_cleaning_step_size = math.ceil(0.01 * len(train_df) + 0.01 * len(test_df))
        ...
        new_clean_partial_train_df, temp_dirty_records, train_df_polluted, cleaning_protocol = clean_data(cleaning_candidates_train_index, train_df, train_df_polluted, clean_partial_train_df, error_map_train_df)
        ...
        used_budget = 0
        print(f'Cleaning protocol Train: {cleaning_protocol}')
        for feature in cleaning_protocol.keys():
            if feature not in global_cleaning_protocol:
                global_cleaning_protocol[feature] = {}
            for et in cleaning_protocol[feature].keys():
                if et not in global_cleaning_protocol[feature] or global_cleaning_protocol[feature][et] == 0:
                    global_cleaning_protocol[feature][et] = 0
                    used_budget += recommender.cost_function.calculate_used_budget(feature, et)
                global_cleaning_protocol[feature][et] += cleaning_protocol[feature][et]
                if global_cleaning_protocol[feature][et] >= absolute_cleaning_step_size:
                    global_cleaning_protocol[feature][et] = 0

        new_clean_partial_test_df, temp_dirty_records, test_df_polluted, cleaning_protocol = clean_data(cleaning_candidates_test_index, test_df, test_df_polluted, clean_partial_test_df, error_map_test_df)
        ...
        print(f'Cleaning protocol Test: {cleaning_protocol}')
        ...
        write_result_to_db(iteration, ds_name, ml_algorithm_str, error_type.__name__, pre_pollution_setting_id, database_engine, number_of_cleaned_cells, used_budget, f1_score)
        iteration += 1
        index_list = []
        while BUDGET > 0:
            used_budget = 0
            ...
            new_clean_partial_train_df, temp_dirty_records, train_df_polluted, cleaning_protocol = clean_data(cleaning_candidates_train_index, train_df,
                                                                        train_df_polluted, clean_partial_train_df, error_map_train_df)
            clean_partial_train_df = pd.concat([clean_partial_train_df, new_clean_partial_train_df])
            tracked_cleaned_records_df = pd.concat([tracked_cleaned_records_df, new_clean_partial_train_df])
            tracked_dirty_records_df = pd.concat([tracked_dirty_records_df, temp_dirty_records])
            ...
            new_clean_partial_test_df, temp_dirty_records, test_df_polluted, cleaning_protocol = clean_data(cleaning_candidates_test_index, test_df,
                                                                       test_df_polluted, clean_partial_test_df, error_map_test_df)
            clean_partial_test_df = pd.concat([clean_partial_test_df, new_clean_partial_test_df])
            tracked_cleaned_records_df = pd.concat([tracked_cleaned_records_df, new_clean_partial_test_df])
            tracked_dirty_records_df = pd.concat([tracked_dirty_records_df, temp_dirty_records])
        ...
        print(f'Budget used: {original_budget - BUDGET}')
...
def clean_data(cleaning_candidates_index, df_clean, df_polluted, clean_partial_df, error_map_df):
    ...
    for record_index in cleaning_candidates_index:
        ...
        for feature in cleaning_candidates_index[record_index]:
            if feature not in cleaning_protocol:
                cleaning_protocol[feature] = {}
            if new_clean_partial_df.at[record_index, feature] != df_clean.at[record_index, feature]:
                current_error_type = error_map_df.at[record_index, feature]
                if current_error_type not in cleaning_protocol[feature]:
                    cleaning_protocol[feature][current_error_type] = 0
                cleaning_protocol[feature][current_error_type] += 1
    ...
    return new_clean_partial_df, dirty_partial_df, df_polluted, cleaning_protocol
...
def get_importance_based_cleaning_candidates(encoder, ds_name, ml_algorithm, polluted_df, dirty_indexes_per_feature, dirty_records, number_of_cells_per_iteration, clean_df, delta_x, delta_y):
    ...
    for dirty_record_index in dirty_records:
        ...
        # set column names to be the same as clean_df
