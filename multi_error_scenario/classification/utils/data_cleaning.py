...
class SimulationCleaning(CleaningInterface):
    def clean(self, feature: str, **kwargs):
        ...
        error_map_train: DataFrame = kwargs.get('error_map_train')
        error_map_test: DataFrame = kwargs.get('error_map_test')
        ...
        if cleaning_candidates_train is not None:
            ...
            indexes = error_map_train[error_map_train[feature].str.contains(error_type.__name__)].index.tolist()
            dirty_indexes_train = [index for index in dirty_indexes_train if index in indexes]
            clean_indexes_train = mismatches.index.difference(dirty_indexes_train)
            mismatches.loc[clean_indexes_train] = False
        ...
        if cleaning_candidates_test is not None:
            ...
            indexes = error_map_test[error_map_test[feature].str.contains(error_type.__name__)].index.tolist()
            dirty_indexes_test = [index for index in dirty_indexes_test if index in indexes]
            clean_indexes_test = mismatches.index.difference(dirty_indexes_test)
            mismatches.loc[clean_indexes_test] = False
