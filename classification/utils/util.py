...
def load_and_prepare_data(table_name, database_url):
    ...
    # replace na values in used_budget with 0
    df['used_budget'] = df['used_budget'].fillna(0)
    #df['used_budget'] = df['iteration']  # fixed for our linear cost function
    df['used_budget'] = df['used_budget'].astype(int)
    # group by pre-pollution setting, sort by iteration, then cumsum used_budget
    df['used_budget'] = df.groupby(['pre_pollution_setting_id'])['used_budget'].cumsum()
    # filter for pre pollution setting == 1 and print df
    # Applying routine for each pre_pollution_setting_id
...
def plot_performance_agg(agg_results_df, title=''):
    ...
    if st.checkbox('Toggle matplotlib visibility', key=title):
        ...
        # Create a Seaborn bar plot on the specified ax with error bars
        ...
        # Setting the plot details on the ax
        ax.set_xlabel('Used Budget')
        ax.set_ylabel('F1 Difference')
        ax.grid(axis='y', linestyle='--', alpha=0.7)

        # Set x-ticks starting from the 20th entry and then every 20 entries
        ticks = np.arange(19, len(agg_results_df), 20)  # 19 because Python indexing starts at 0
        ax.set_xticks(ticks)
