...
best_params: dict = {}
f1_scores: dict = {}
DS_NAMES = ['adult.csv', 'cmc.data', 'Credit', 'EEG.arff', 'SouthGermanCredit.csv', 'TelcoCustomerChurn.csv', 'Titanic']
ERROR_TYPES = [CategoricalShiftModifier, MissingValuesModifier, GaussianNoiseModifier, ScalingModifier]
EXPERIMENTS = [SupportVectorMachineExperiment, KNeighborsExperiment, MultilayerPerceptronExperiment, GradientBoostingExperiment]
...
for ds_name in DS_NAMES:
    ...
    for error_type in ERROR_TYPES:
        ...
        best_params[ds_name][error_type.__name__] = {}
        f1_scores[ds_name][error_type.__name__] = {}
        ...
        for pollution_setting in pre_pollution_settings:
            ...
            best_params[ds_name][error_type.__name__][str(pre_pollution_setting_id)] = {}
            f1_scores[ds_name][error_type.__name__][str(pre_pollution_setting_id)] = {}
            ...
            if is_empty:
                ...
                continue
            train_df_polluted, test_df_polluted = ap.get_current_polluted_training_and_test_df(filtered_history_df,
                                                                                               ds_name, pollution_setting)
            ...
            for experiment in EXPERIMENTS:
                ...
                best_params[ds_name][error_type.__name__][str(pre_pollution_setting_id)][exp.__class__.__name__] = exp.best_params
                f1_scores[ds_name][error_type.__name__][str(pre_pollution_setting_id)][exp.__class__.__name__] = exp.run()
                print(f'Finished {exp.__class__.__name__} for {ds_name}')
                print(f'F1 score: {f1_scores[ds_name][error_type.__name__][str(pre_pollution_setting_id)][exp.__class__.__name__]}')
                print(f'Best parameters: {best_params[ds_name][error_type.__name__][str(pre_pollution_setting_id)][exp.__class__.__name__]}')
                print('---------------------------------')
